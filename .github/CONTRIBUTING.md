...
First off, **thank you** for wanting to improve **📦 Value Object Pattern** package! Whether you're fixing a typo or building a whole new feature, your help makes the library better for everyone.

## 🏃🏻‍➡️ Before You Start

1. **Be kind, inclusive, and patient**: we follow our [`🧭 Code of Conduct`](https://github.com/adriamontoto/value-object-pattern/blob/master/.github/CODE_OF_CONDUCT.md).
2. **Search first**: check for existing [issues](https://github.com/adriamontoto/value-object-pattern/issues) before opening a new one.
3. **Security issues**: report privately via our [`🔐 Security Policy`](https://github.com/adriamontoto/value-object-pattern/blob/master/.github/SECURITY.md); **do not** raise a public issue for vulnerabilities.
...
```bash
git clone git+ssh://git@github.com/<your-username>/value-object-pattern.git
```
...
```bash
git add .
git commit -m "feat(primitives): implement IntegerValueObject type validation" -S --signoff  # we only accept signed and signed-off commits
```
...
9. **Open a Pull Request**: Open a pull request against the `master` branch and fill out our [`pull request template`](https://github.com/adriamontoto/value-object-pattern/blob/master/.github/pull_request_template.md).
...
- **Structure**: Each commit message must consist of a type, an optional scope, and a concise description (e.g., `feat(primitives): implement IntegerValueObject type validation`).
...
- Use our [`pull request template`](https://github.com/adriamontoto/value-object-pattern/blob/master/.github/pull_request_template.md).
